...
# File: azure_available_regions.md
...
# File: go_and_ruby_sqlite_setup.md
...
**Motivation**: Use FTS5 in SQLite for faster searches.

---

## Use FTS5 in SQLite

The task is to set up FTS5 for SQLite.
...
Note that headers are required. As of August 2024 Wikipedia started enforcing [T400119](https://wikitech.wikimedia.org/wiki/Robot_policy) which requires an user agent in requests.
...
There are many similar stories carrying huge sums in the headlines.
...
*What are the positives about the waterfall model?*
...
Agile development should not eliminate those steps. It just allows one to move back and forth between the steps. 
...
# File: 04._debug_docker_compose.md
...
First confirm the missing resources in the UI:

1. [teams](https://github.com/orgs/testterraformprovider/teams)

2. [repositories](https://github.com/orgs/testterraformprovider/repositories)

3. [branch rules](https://github.com/testterraformprovider/terraform_provisioned_repo/settings/branches)
...
The check the three links again.
...
Remember to update `location` to one of your available regions.

Check below to find the corresponding name to the programmatic name:
...
The `id` key-value pair is the subscription ID.
...
Inside of the `azurerm_linux_virtual_machine` block, add the following:
...
Azure for Students at EK doesn't support the creation or management of a **Service Principal**.
...
2. Display the settings go to **Advanced security**.
...
2. Locate **Dependabot version updates** and click "Enable" to open a new file editor with pre-poplulated contents. The file is called `dependabot.yml`.
...
Ensure that you use python version `3.x.x`. In the slides `python` is my alias for `python3`. Run `--version` to confirm.
...
# File: 04._incident_response_postmortem.md
...
8. Success! You have now installed a plugin for Grafana. Here is how to set it up in your dashboard including styling the background color, font size etc.:
